...
# http://www2.stat.duke.edu/~st118/Software/burstHSMM/R/burstHSMM.R
fit.hsmm <-
  function(y,
           scan = c(5, 25),
           dur.shape = c(1, 1),
    ...
    out <- list(
      states = matrix(out$states, nrow = n),
      runs = out$runs,
...
plot.hsmm <-
  function(out,
    ...
    burst.step <-
      stepfun(time.stamps, c(p.states, p.states[length(p.states)]))
    plot(
      burst.step,
      do.points = FALSE,
      ann = FALSE,
    ...
    d.nb <-
      (1 - p.b) * apply(apply(pars[3:4,], 2, get.d, g = g), 1, mean)
    d.mix <- d.b + d.nb
    ...
    title(
      main = paste("Fit from", out$method, "model"),
      outer = TRUE,
      line = -2
    )
...
summary.hsmm <-
  function(out,
    ...
    p.states <- 1 - apply(out$states[, subset], 1, mean)
    parsCI <-
      apply(pars[, subset], 1, quantile, c(0.025, 0.25, 0.5, 0.75, 0.975))
    ix <- 1:8
...
simu.hsmm <- function(total.time, pars = NULL) {
  ...
  M <-
    4 * ceiling(total.time / (pars[1] * pars[2] + pars[3] * pars[4]))
  N <-
    4 * ceiling(total.time / (pars[5] * pars[6] + pars[7] * pars[8]))
  
  out <- .C(
    "rspike",
    as.double(pars),
    as.double(total.time),
    states = integer(N),
    spikes = double(N),
    nspikes = integer(1),
    transitions = double(M),
    ntrans = integer(1)
  )
  
  return(
    list(
      spikes = 1e3 * out$spikes[1:out$nspikes],
